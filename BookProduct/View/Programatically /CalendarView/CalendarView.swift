...
open class CalendarView: UIView {
    ...
    open var maxDate = Date().dateByAddingDay(365) {
        didSet {
    ...
    open var imagePreviousName = "ic_arrow_left_blue.png" {
        didSet {
            buttonPrevious.setImage(UIImage(named: imagePreviousName), for: UIControl.State())
        }
    }
    
    open var imageNextName = "ic_arrow_right_blue.png" {
        didSet {
            buttonNext.setImage(UIImage(named: imageNextName), for: UIControl.State())
        }
    }
